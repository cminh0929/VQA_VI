...
# --- Cached model instances (M5: load once, reuse across predictions) ---
_modular_model = None
...
def _get_modular_model():
    global _modular_model, _modular_tokenizer
    if _modular_model is None:
        _modular_tokenizer = AutoTokenizer.from_pretrained('vinai/phobert-base')
        _modular_model = ModularVQA(config, decoder_type='lstm', vocab_size=len(_modular_tokenizer)).to(device)
        ckpt = os.path.join(config.CHECKPOINT_DIR, "modular_lstm_epoch1.pt")
        if os.path.exists(ckpt):
            _modular_model.load_state_dict(torch.load(ckpt, map_location=device))
        else:
            raise FileNotFoundError("Chưa có checkpoint Hướng A. Vui lòng train trước.")
        _modular_model.eval()
    return _modular_model, _modular_tokenizer
...
def predict(image, question, model_type):
    ...
    if model_type == "Modular (Direction A)":
        try:
            model, tokenizer = _get_modular_model()
        ...
        try:
            model, processor = _get_paligemma_model()
